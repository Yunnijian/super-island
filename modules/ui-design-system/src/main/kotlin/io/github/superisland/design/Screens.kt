...
/** Notification permission and HyperOS focus-protocol status are kept in one repair page. */
@Composable
fun FocusNotificationCapabilityScreen(
    ...
    AppScaffold(
        ...
        Column(
            ...
            AppInfoCard("通知权限", notificationStatus)
            AppInfoCard("焦点通知状态", focusProtocolStatus)
            if (showNotificationPermissionAction) {
                AppPrimaryButton("允许通知", onClick = onRequestNotificationPermission)
            }
            AppSecondaryButton("刷新能力状态", onClick = onRefresh)
            AppTextButton("返回焦点通知测试", onClick = onBackToFocusTest)
        }
    }
}

/** All test-event actions are together, away from permission and capability explanations. */
@Composable
fun FocusNotificationEventScreen(
    ...
    AppScaffold(
        ...
        Column(
            ...
            AppInfoCard("发送条件", if (canPublish) "系统通知已允许，可以发送测试事件" else "请先完成系统权限设置")
            AppPrimaryButton(
                text = "发送测试事件",
                enabled = canPublish,
                onClick = onPublish,
            ...
            AppSecondaryButton("结束事件", onClick = onCancel)
            AppTextButton("返回焦点通知测试", onClick = onBackToFocusTest)
        }
    }
}
